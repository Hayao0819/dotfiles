...
stdenv.mkDerivation (finalAttrs: {
  ...
  postUnpack =
    let
      # Starting with linux-sgx 2.29 the SDK simulation build pulls headers
      # from `external/dcap_source/prebuilt/openssl/inc`, which normally come
      # from this prebuilt tarball fetched by the (disabled) download script.
      dcap = rec {
        version = "1.26";
    ...
    ''
      ...
      # Sanity check the dcap prebuilt version expected by upstream matches our pin.
      grep -q 'dcap_version=${dcap.version}' "$src/external/dcap_source/QuoteGeneration/download_prebuilt.sh" \
